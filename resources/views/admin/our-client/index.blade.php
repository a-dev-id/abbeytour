...
<x-app-layout>
    <div class="container-fluid">
        <div class="row">
                        ...
                        <table id="list" class="table-striped w-100 table">
                            ...
                            <tbody>
                                @foreach ($our_clients as $oc)
                                    <tr>
                                        <td class="fw-bold fs-4">{{ $oc->title }}</td>
                                        <td>
                                            @if ($oc->featured == 'on')
                                        ...
                                        <td class="text-end">
                                            <a href="{{ route('our-client.edit', [$oc->id]) }}" class="btn btn-warning btn-sm text-white"><i class="fa-solid fa-edit"></i></a>
                                            <button class="btn btn-danger btn-sm text-white" data-coreui-toggle="modal" data-coreui-target="#deleteModal{{ $oc->id }}"><i class="fa-solid fa-trash"></i></button>

                                            <div class="modal fade" id="deleteModal{{ $oc->id }}" tabindex="-1" aria-labelledby="deleteModal{{ $oc->id }}Label" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <form method="POST" action="{{ route('our-client.destroy', [$oc->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <div class="modal-header bg-danger">
                                                                <h5 class="modal-title text-white" id="deleteModal{{ $oc->id }}Label"><i class="fa-solid fa-triangle-exclamation"></i> Warning</h5>
                                                                <button type="button" class="btn-close text-white" data-coreui-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                Are you sure want to delete this <strong>"{{ $oc->title }}"</strong> ?
                                                            </div>
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('our-client.store') }}" class="col-4">
                @csrf
                <div class="col-12">
                    <div class="card mb-5">
